...
{
    // fixed parameters
    cameraMatrix1 = (cv::Mat_<double>(3, 3) << 1767.37847031424, 0, 0,
                     0, 1767.62819410665, 0,
                     344.891889741740, 684.467032443013, 1);
    cameraMatrix1 = cameraMatrix1.t();      // Intrinsic matrix is the transposed matrix of what MATLAB gets.
    distCoffs1 = (cv::Mat_<double>(1, 5) << -0.0878174462561571, 0.149136127528808,
                  0.00241323034256646, -0.00116556205234743, 0.198115461781876);

    cameraMatrix2 = (cv::Mat_<double>(3, 3) << 1735.10135605985, 0, 0,
                     0, 1736.28293418657, 0,
                     324.603238826833, 649.158538735089, 1);
    cameraMatrix2 = cameraMatrix2.t();      // Intrinsic matrix is the transposed matrix of what MATLAB gets.
    distCoffs2 = (cv::Mat_<double>(1, 5) << -0.0935558166901970, 0.100354985611543,
                  0.00151671799822277, -0.00300291727072402, 0.350330539541730);

    R12 = (cv::Mat_<double>(3, 3) << 0.999927134674646, 0.000323577387783399, -0.0120673376942240,
           -0.000246890970626450, 0.999979771075417, 0.00635582290549546,
           0.0120691501855328, -0.00635238046967536, 0.999906987112385);
    R12 = R12.t();                  // Rotation matrix is the transposed matrix of what MATLAB gets.
    T12 = (cv::Mat_<double>(3, 1) << -85.3253712806288, 4.94668944027864, -21.6625789660096);
}
